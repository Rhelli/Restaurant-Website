...
const generateMenu = () => {
  ...
  const menuShortcuts = () => {
    ...
    for (let i = 0; i < menuShortcutsArray.length; i++) {
      const element = generator.htmlGenerator('div', `menu-shortcut-${i}`, `menuShortcut${i}`);
      const text = generator.textGen('a', `${menuShortcutsArray[i]}`);
  ...
  const sandwichMenu = () => {
    const unparsedSandwichNames = ["Chicken*Salad*&*'Slaw*", 'Croque*Monsieur*Deluxe', 'Vegan*Power*Wrap', "Shooter's*Sandwich", 'B.A.B.', 'Fully*Loaded*Tacos', 'Crispy*Chicken*Panini', 'Croissant*Au*Mediterranean'];
    const unparsedSandwichDescriptions = [
      ...
      'The*lighter*choice.*Our*homemade*viennoiserie*based*croissant*is*filled*with*corned*beef,*sweet*roma*tomatoes,*curled*lettuce,*a*potato*and*fat-reduced*mozzarella*omelette*and*a*spread*of*homemade*light*mayo.',
    ];
  ...
  const sidesMenu = () => {
    ...
    const unparsedSidesDescriptions = [
      'A*sumptuously*sweet*and*seasonal*side*salad*for*you.*Delectable*Fuji*apple*slices,*curled*lettuce,*spring*onions,*white*onion*and*fresh*cucumber.*A*perfect*accompaniment*to*spicy*or*intense*dishes.',
      ...
      'Our*most*nutritious*and*powerful*salad*yet.*Fresh*coriander,*avocado,*sliced*chillies,*peanuts,*button*mushrooms,*fresh*cucumber,*tomatoes*and*black*sesame*seeds.*Served*with*a*wedge*of*lemon*and*our*Seven*Islands*dressing.',
  ...
  const dessertMenu = () => {
    const unparsedDessertNames = [
      "Auntie*Ellie's*Double*Dark*Chocolate*&*Peanut*Cookies",
      'English*Roll',
      'Vanilla,*Chocolate*&*Rocky*Road*Ice*Cream',
      'Homemade*Macrons',
    ];
    const unparsedDessertDescriptions = [
      ...
      'Our*take*on*a*Austrian*classic.*Soft,*doughy*cinnamon*and*carrot*infused*dough*with*a*sweet*vanilla*and*buttercream*icing.*Perfect*for*those*chilly*winter*afternoons*with*a*warm*drink.',
      'Homemade*rocky*road*with*pecans,*marshmallows,*infused*into*our*40%*dark*chocolate*/*60%*white*milk*chocolate*ice-cream.*Comes*in*a*standard*2*scoop*serving*with*an*optional*extra*scoop*for*£1.*Cones*£1.50.',
      'Our*Macrons*are*made*fresh*daily*-*so*no*stale*Macrons*for*anyone*here!*Flavour*choices*include*Pineapple*Jam*(Yellow),*Lavender*&*Honey*Marscapone*(Purple),*Blueberry*Jam*Cheesecake*(Blue)*and*Almond-Raspberry*(Red). Come in servings of fours. Additional Macrons are £1.50.',
    ];
